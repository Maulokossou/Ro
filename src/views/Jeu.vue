...
function rotate(e: MouseEvent) {
    ...
    if (paires.value.length < 2) {
        ...
        if (paires.value.length === 2) {
            ...
            const datasetId1 = paires.value[0].getAttribute("data-id");
            const datasetId2 = paires.value[1].getAttribute("data-id");
...
function recommencer() {
    ...
    const restartButton = document.querySelector("#restart") as HTMLButtonElement | null;
    if (pairesTrouver.value === 9 && restartButton) {
        clearInterval(timer.value);
...
function reset() {
    affichageInitialEnCours.value = true;

    cardRefs.value.forEach((cardRef) => {
        if (cardRef) { // Vérifier si cardRef est défini
            ...
            <main>
                ...
                <div class="cards">
                    ...
                    <div class="card" v-for="(element, i) in pairesImages" :key="i" :data-id="element.id"
                        @click.self="rotate" :ref="ref => cardRefs[i] = ref as HTMLElement">
